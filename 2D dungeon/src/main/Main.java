...
public class Main {
	...
	private void createRooms(int i) {
		...
		for(int r = 0; r < i; r++){
			int xsize = roundm((int)(randomBiased(0.8)*((room_max_size-room_min_size)*Tile.size))+(room_min_size*Tile.size));
			int ysize = roundm((int)(randomBiased(0.8)*((room_max_size-room_min_size)*Tile.size))+(room_min_size*Tile.size));
	...
	public void render(Graphics2D g) {
		if(grid != null){
			for(Room e : new ArrayList<Room>(rooms)){
